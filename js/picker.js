...
function restore() {
	chrome.storage.local.get({
		...
		document.querySelector(`input[value="${items.output}"]`).setAttribute('checked', true)
	});
}
...
const _onLoadListener = function () {
	...
	restore()
	document.querySelectorAll('input[type=radio][name=output]').forEach(input => {
...
function init(output) {
	...
	Object.entries(colorsJson.mainColors).forEach(([key, data]) => {
		...
		Object.entries(data.alternates).forEach(([keys, datas]) => {
			...
			colorsettoadd.setAttribute('data-clipboard-text', color(datas, output))
			...
			document.querySelector(`#colorset-${key}`).appendChild(colorsettoadd)
		})
	})
	...
	document.querySelectorAll('.color').forEach(color => {
		color.addEventListener('click', e => {
			...
			clipboard.on('success', e => {
				fadeIn(alert)
				alert.innerHTML = `<strong>SUCCESS:</strong> Copied to Clipboard ${e.text}`
				setTimeout(() => {
					fadeOut(alert)
				}, 1500)
			})
			clipboard.on('error', _ => {
				fadeIn(alert)
				alert.innerHTML = '<strong>ERROR:</strong> Cannot copy to Clipboard'
				setTimeout(() => {
					fadeOut(alert)
				}, 1500)
			})
